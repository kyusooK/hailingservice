<template>
    <v-card outlined @click="openDialog">
        <v-card-title>
            Driver : {{ referenceValue ? referenceValue._links.self.href.split('/').pop() : '' }}
        </v-card-title>
        ...
        <v-dialog v-model="pickerDialog">
            <v-card>
                ...
                <v-card-text>
                    <DriverPicker v-model="value" @selected="pick"/>
                </v-card-text>
                <v-card-actions>
                    <v-spacer></v-spacer>
        ...
        methods: {
            ...
            openDialog() {
                ...
                if(this.editMode) {
                    ...
                    this.$router.push(path + this.value.driverId);
                }
            },
            async pick(val){
                this.newValue = val;
                var path = '/drivers';
                var temp = await axios.get(axios.fixUrl(path + '/' + val.driverId));
                if(temp.data) {
                    this.referenceValue = temp.data;
                }
