<template>
    <v-card outlined>
        ...
        <v-card-title v-if="value._links">
            매칭정보 # {{decode(value._links.self.href.split("/")[value._links.self.href.split("/").length - 1])}}
        </v-card-title >
        <v-card-title v-else>
            매칭정보
        </v-card-title >        
        ...
        <v-card-text style="background-color: white;">
            <UserId v-if="!editMode" offline label="사용자정보 ID" v-model="value.userId" :editMode="false" @change="change"/>
            <DriverId v-if="!editMode" offline label="운전자정보 ID" v-model="value.driverId" :editMode="false" @change="change"/>
        ...
        methods: {
            ...
            async save(){
                try {
                    ...
                    if(!this.offline) {
                        if(this.isNew) {
                            temp = await axios.post(axios.fixUrl('/matchings'), this.value)
                        } else {
